...
quantifier elimination algorithms can decide formulas over pure linear arithmetic in doubly exponential time.
However, the general case of quantified formulas having (uninterpreted) function symbols is undecidable.
In spite of the theoretical challenges, many quantified formulas that arise in practice
can be handled efficiently by modern automated theorem provers.
...
Satisfiability Modulo Theories (SMT) solvers such as CVC~\cite{stump2002cvc}, Yices~\cite{dutertre2006yices}, and Z3~\cite{Z3}, 
...
This section gives an brief review of how DPLL(T)-based solvers check the satisfiability of
inputs with quantified formulas, which can be summarized by the abstract procedure $\funcsmtsolve$ in Figure~\ref{fig:smtq}.
...
Otherwise, in the case that $\Gamma$ is satisfiable, the SAT solver will return a set of literals $M$ that propositionally entail it.
...
In line 5, we the $T$-satisfiability of quantifier-free part of $M$.
If $M$ is unsatisfiable, then we add the clause $\neg C$ to $\Gamma$ in line 6 and repeat,
where $C$ is an unsatisfiable subset of $E$ ($\neg C$ is often referred to as a conflict or blocking clause).
...
satisfiable modulo $\uflia$, or adds additional formulas to $\Gamma$. 
...
Overall, {\bf cvc4+ci} solves 807 more benchmarks than {\bf cvc4}
while adding a total of 847.1 million fewer instantiations for doing so.
...
showing a number of difficult benchmarks can be solved by conflict-based instantiation where E-matching fails.
...
Hence, we will be forced to choose a new candidate model if model-based quantifier instanatiation is invoked again.
\qed
\end{example}
\begin{example}
...
all uninterpreted sorts as finite sets, and complete for fragments
...
over linear arithmetic is given in~\cite{InstLA2016}. 
In this approach, the basic idea for instance selection is the following.
...
In other words, instantiations are selected based on the current maximal lower bound for $k$.
Alternatively, we may add instantiations based on the current minimal upper bound.
...
The aforementioned approach for instantiation for linear arithmetic is often highly efficient as
shown in~\cite{DBLP:conf/lpar/BjornerJ15,InstLA2016}.
We have shown one option for selecting instantiations based on maximal lower or minimal upper bounds,
...
which intuitively can be understood as enumerating disjuncts in Ferrante and Rackoff's method for quantifier elimination~\cite{FerranteRackoff79ComputationalComplexityLogicalTheories}.
...
In all of the aforemtnioned cases, we may argue that the set of possible instantiation lemmas returned by this strategy is finite.
...
There is also potential for incorporating shapes beyond almost constant functions
...
We expect analogous techniques for selecting instantiations to be helpful here.
...
As mentioned, E-matching techniques
