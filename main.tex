...
\item Add the lemmas $\forall x\, \varphi[\vec x] \Rightarrow \varphi[\vec t_{1}], \ldots, \forall x\, \varphi[\vec x] \Rightarrow \varphi[\vec t_{n}]$ to $L$.
\end{enumerate}
\end{enumerate}
...
We then add instantiation lemmas corresponding to each of these pairs.
We demonstrate this technique in the following examples.

\begin{example}
Let $E$ be the set $\{ P(a), \neg P(b), R(c), \neg R(a), S(e) \}$, and $Q$ be the set $\{ \forall x\, P( x ) \vee R( x ) \}$.

\end{example}

\begin{example}
E-matching also takes into account 
\end{example}
[Extension to EUF]
...
There is no standard  selecting which patterns to process with E-matching.
