...
As mentioned, E-matching often generates too many instantiations, thereby degrading the performance of the SMT solver.
To address this shortcoming, we may use a technique which we refer to as \emph{conflict-based quantifier instantiation}~\cite{},
which has been implemented in a few current SMT solvers~\cite{}.
The basic idea is to identify quantifier instantiations that are in conflict with the current context, 
and consider these with higher priority than others.

In detail, given a quantified formula $\forall \vec x\, \varphi[ \vec x ]$ in $Q$,
we say that $\varphi[ \vec t ]$ is a \emph{conflicting instance} for $( E, Q )$ if $E, \varphi[ \vec t ] \models_T \bot$.
If a single conflicting instance exists for $(E, Q )$, then we already know that $E \cup Q$ is unsatisfiable,
and may return \emph{only} that instance.
...
\end{array}
\]
In other words, since $P( b ) \in E$, we learn no new information from the first instance,
whereas since $\neg P( d ), \neg R( d ) \in E$, we learn $\bot$ from the fourth instance.
In this example, $P( d ) \vee R( d )$ is a conflict instance for $( E, \{ \psi \} )$.
Conflict-based instantiation would return this lemma only and discard the others.
\qed
\end{example}

This technique can be extended to find instantiations that are conflicting taking into account equality and uninterpreted functions.
