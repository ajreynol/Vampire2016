...
The common thread thus far has been that {\bf determining the satisfiability of universally quantified formulas in presence
...
then satisfiability can be established by a quantifier elimination procedure~\cite{}.

Recent work has adapted quantifier elimination techniques in the context of SMT solving~\cite{},
and has been implemented in SMT solvers such as Z3~\cite{} and \cvc~\cite{},
as well as tools that use SMT solvers as a backend~\cite{}.
Abstractly, these approaches can 
...
There is still a lot of room to improve upon current implementations of quantifier instantiation in SMT solvers,
both in terms of engineering and in theory.
For the former, it has yet to be explored how conflict-based instantiation can be implemented eagerly,
and if it can be efficiently applied in the presence of background theories.

For the latter, there are a number of opportunities to extend SMT solvers with efficient dedicated support for
new fragments of interest.
For example, we have seen that specialized approaches for quantifier instantiation in the spirit of Section~\ref{sec:cegqi} 
are highly effective for theories such as linear real and integer arithmetic~\cite{}.
There is potential for improving support for quantified formulas over fixed-width bitvectors,
where note that the bit-precise versions of arithmetic operators are part of the signature supported by many SMT solvers.
...
which a number of SMT solvers have recently added support for.
